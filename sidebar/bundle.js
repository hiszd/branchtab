...
document.addEventListener('DOMContentLoaded', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!hasInit) {
        ...
        webextension_polyfill_1.default.storage.local.get(null).then(options => {
            const newtabsearch = options["newtabsearch"];
            const newtabbehavior = options["newtabbehavior"];
            newtabsearch ? console.log("newtabsearch: " + newtabsearch) : webextension_polyfill_1.default.storage.local.set({ newtabsearch: "New Tab" });
            newtabbehavior ? console.log("newtabbehavior: " + newtabbehavior) : webextension_polyfill_1.default.storage.local.set({ newtabbehavior: "0" });
        });
        hasInit = true;
    }
}));
            ...
            cache === null || cache === void 0 ? void 0 : cache.forEach(tab => {
                ...
                if (ft) {
                    if (!tab.parentUrl) {
                        ...
                        const parent = rtrn.find(t => t.url === tab.parentUrl);
                        if (parent) {
